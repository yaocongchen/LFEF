...
def acc_miou(model_output,mask, smooth=1):             #smooth避免分母為0
    model_output = S(model_output)
    intersection = torch.sum(model_output * mask)       #算出交集
    union = torch.sum(model_output) + torch.sum(mask)   #2*考慮重疊的部份 #計算模型輸出和真實標籤的Dice係數，用於評估二元分割模型的性能。參數model_output和mask分別為模型輸出和真實標籤，smooth是一個常數，用於避免分母為0的情況。
    return torch.mean((2. * intersection + smooth) / (union + smooth))
# def true_positive_rate(model_output,mask):
...
class Calculate():
    ...
    def Calculations_and_parameters(self):
            macs, params = get_model_complexity_info(self.model, (3, 256, 256), as_strings=True, print_per_layer_stat=False, verbose=True)     #計算模型計算量與參數量 print_per_layer_stat：列出每一層的參數量與計算量
            print('{:<30}  {:<8}'.format('Computational complexity(FLOPs): ', macs))
