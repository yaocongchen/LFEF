...
import models.CGNet_2_erfnet31_13_3113_sem_cam as network_model  # import self-written models 引入自行寫的模型
import utils
...
def train_epoch(model, training_data_loader, device, optimizer, epoch):
    ...
    return RGB_image, mask_image, output,output2
...
def valid_epoch(model, validation_data_loader, device, epoch):
    ...
    for RGB_image, mask_image in pbar:
        ...
        loss = utils.loss_two.CustomLoss(output, output2,mask_image, mode="val")
        iou = utils.metrics.IoU(output, mask_image)
        # iou_s = utils.metrics.Sigmoid_IoU(output, mask_image)
        # dice_coef = utils.metrics.dice_coef(output, mask_image, device)
...
def main():
    ...
    for epoch in range(start_epoch, args["epochs"] + 1):
        train_RGB_image, train_mask_image, train_output ,train_output2= train_epoch(
        ...
        if args["save_train_image"] != "no":
            ...
            torchvision.utils.save_image(
                train_output2, "./training_data_captures/" + "last_output2_" + ".jpg"
            )
        ...
        if args["save_validation_image_last"] != "no":
            ...
            torchvision.utils.save_image(
                output2, "./validation_data_captures/" + "last_output2_" + ".jpg"
            )
        ...
        if args["wandb_name"] != "no":
            ...
            if args["save_train_image"] != "no":
                ...
                wandb.log(
                    {
                        "train_output2": wandb.Image(
                            "./training_data_captures/" + "last_output2_" + ".jpg"
                        )
                    }
                )
            if args["save_validation_image_last"] != "no":
                ...
                wandb.log(
                    {
                        "last_output2": wandb.Image(
                            "./validation_data_captures/" + "last_output2_" + ".jpg"
                        )
                    }
                )
        ...
        if mean_miou > save_mean_miou:
            ...
            if args["save_validation_image_bast"] != "no":
                ...
                torchvision.utils.save_image(
                    output2, "./validation_data_captures/" + "best_output2_" + ".jpg"
                )
            ...
            if args["wandb_name"] != "no":
                ...
                if args["save_validation_image_bast"] != "no":
                    ...
                    wandb.log(
                        {
                            "best_output2": wandb.Image(
                                "./validation_data_captures/" + "best_output2_" + ".jpg"
                            )
                        }
                    )
