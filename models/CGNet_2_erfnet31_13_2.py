...
class Net(nn.Module):
    ...
    def __init__(self, classes=1, M=3, N=21, dropout_flag=False):
        ...
        self.level1_2 = non_bottleneck_1d(32, 0.03, 4)

        self.sample1 = InputInjection(1)  # down-sample for Input Injection, factor=2
        self.sample2 = InputInjection(2)  # down-sample for Input Injiection, factor=4

        self.b1 = BNPReLU(32 + 3 + 32)

        # stage 2
        self.level2_0 = ContextGuidedBlock_Down(
            32 + 3 + 32, 64, dilation_rate=2, reduction=8
        )
        ...
        self.bn_prelu_3 = BNPReLU(256 + 32)

        if dropout_flag:
            print("have droput layer")
            self.classifier = nn.Sequential(
                nn.Dropout2d(0.1, False), Conv(256+32, classes, 1, 1)
            )
        else:
            self.classifier = nn.Sequential(Conv(256+32, classes, 1, 1))
    ...
    def forward(self, input):
        ...
        output0_1 = self.level1_1(output0)
        output0_2 = self.level1_2(output0_1)
        inp1 = self.sample1(input)
        inp2 = self.sample2(input)
        inp3 = self.sample2(output0_2)   

        # stage 2
        output0_cat = self.b1(torch.cat([output0_2, inp1,output0_1], 1))
