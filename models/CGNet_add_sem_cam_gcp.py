...
class GCP(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.gcp = DownUnit(in_ch, 512)  # 16 x 16 x 256
        self.conv11_in512_out128 = nn.Sequential(
            nn.Conv2d(512, out_ch, kernel_size=(1, 1), padding="same"),
            nn.BatchNorm2d(out_ch),
        )
        self.gavgpl = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        f14 = self.gcp(x)
        f15 = self.conv11_in512_out128(f14)
        f16 = self.gavgpl(f15)
