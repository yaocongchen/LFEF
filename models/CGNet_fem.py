...
class CGNet(nn.Module):
    ...
    def __init__(self, in_chs, M=3, N=21):
        """
        args:
        ...
        self.level1_0 = ConvBNPReLU(in_chs, 32, 3, 2)  # feature map size divided 2, 1/2
        ...
        self.b1 = BNPReLU(32 + in_chs)

        # stage 2
        self.level2_0 = ContextGuidedBlock_Down(
            32 + in_chs, 64, dilation_rate=2, reduction=8
        )
        ...
        self.bn_prelu_2 = BNPReLU(128 + in_chs)

        # stage 3
        self.level3_0 = ContextGuidedBlock_Down(
            128 + in_chs, 128, dilation_rate=4, reduction=16
        )
...
class Net(nn.Module):
    def __init__(self, classes=1, dropout_flag=False):
        super().__init__()
        ...
        self.fem2 = nn.Sequential(
            nn.Conv2d(16, 32, kernel_size=3, padding=1, stride=2),
            nn.ReLU(inplace=True),
            nn.Sigmoid(),
        )
        self.fem3 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, padding=1, stride=2),
            nn.ReLU(inplace=True),
            nn.Sigmoid(),
        )
        self.fem4 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, padding=1, stride=2),
            nn.ReLU(inplace=True),
            nn.Sigmoid(),
        )

        self.cg_net_16 = CGNet(in_chs=16)
        self.cg_net_32 = CGNet(in_chs=32)
        self.cg_net_64 = CGNet(in_chs=64)
        self.cg_net_128 = CGNet(in_chs=128)

        self.bn_prelu = BNPReLU(4)
        self.classifier = nn.Sequential(Conv(4, 1, 1, 1))
    ...
    def forward(self, input):
        output_fem1 = self.fem1(input)
        output_fem1_cg = self.cg_net_16(output_fem1)

        output_fem2 = self.fem2(output_fem1)
        output_fem2_cg = self.cg_net_32(output_fem2)

        # upsample segmenation map ---> the input image size
        output_fem2_cg = F.interpolate(
            output_fem2_cg,
            output_fem1_cg.size()[2:],
            mode="bilinear",
            align_corners=False,
        )  # Upsample score map, factor=8

        output_fem3 = self.fem3(output_fem2)
        output_fem3_cg = self.cg_net_64(output_fem3)

        # upsample segmenation map ---> the input image size
        output_fem3_cg = F.interpolate(
            output_fem3_cg,
            output_fem1_cg.size()[2:],
            mode="bilinear",
            align_corners=False,
        ...
        output_fem4 = self.fem4(output_fem3)
        output_fem4_cg = self.cg_net_128(output_fem4)

        output_fem4_cg = F.interpolate(
            output_fem4_cg,
            output_fem1_cg.size()[2:],
            mode="bilinear",
            align_corners=False,
        ...
        output = output_fem1_cg + output_fem2_cg + output_fem3_cg + output_fem4_cg
        ...
        # output = self.bn_prelu(
        #     torch.cat(
        #         [output_fem1_cg, output_fem2_cg, output_fem3_cg, output_fem4_cg], 1
        #     )
        # )
