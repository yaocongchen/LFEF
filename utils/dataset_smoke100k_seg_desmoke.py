...
class DatasetSegmentation(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if "_L_" in filename:
            y = f"{self.dataset_dir}/smoke100k-L/smoke_mask/{filename}.{self.mask_extension}"
            z = f"{self.dataset_dir}/smoke100k-L/smoke_free_image/{filename}.{self.mask_extension}"
        elif "_M_" in filename:
            y = f"{self.dataset_dir}/smoke100k-M/smoke_mask/{filename}.{self.mask_extension}"
            z = f"{self.dataset_dir}/smoke100k-M/smoke_free_image/{filename}.{self.mask_extension}"
        elif "_H_" in filename:
            y = f"{self.dataset_dir}/smoke100k-H/smoke_mask/{filename}.{self.mask_extension}"
            z = f"{self.dataset_dir}/smoke100k-H/smoke_free_image/{filename}.{self.mask_extension}"
        ...
        bg_img = imread(free_path)
        bg_img = cv2_brightness_augment(bg_img)
