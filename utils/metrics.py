...
def acc_miou(model_output,mask, smooth=1):             # "Smooth" avoids a denominsator of 0 "Smooth"避免分母為0
    ...
    union = torch.sum(model_output) + torch.sum(mask)   # 2*Consider the overlapping part # Calculate the Dice coefficient of the model output and the real label, which is used to evaluate the performance of the binary segmentation model. The parameters model_output and mask are the model output and the real label respectively, and smooth is a constant used to avoid the case where the denominator is 0.
    return torch.mean((2. * intersection + smooth) / (union + smooth)) # 2*考慮重疊的部份 #計算模型輸出和真實標籤的Dice係數，用於評估二元分割模型的性能。參數model_output和mask分別為模型輸出和真實標籤，smooth是一個常數，用於避免分母為0的情況。
...
class Calculate():
    ...
    def Calculations_and_parameters(self):
            macs, params = get_model_complexity_info(self.model, (3, 256, 256), as_strings=True, print_per_layer_stat=False, verbose=True)     # Calculation model calculation amount and parameter amount print_per_layer_stat: List the parameter amount and calculation amount of each layer
            print('{:<30}  {:<8}'.format('Computational complexity(FLOPs): ', macs))                                                           # 計算模型計算量與參數量 print_per_layer_stat：列出每一層的參數量與計算量
